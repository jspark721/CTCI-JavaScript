...
function LinkedList() {
  ...
  this.add = function(element) {
    ...
    if(head === null) { // if there are no nodes in the linked list yet
      head = node;
    } else { // more than one element in the node
      var currentNode = head; // start at the head node, start at the beginning

      while(currentNode.next) { //while there is a currentNode.next -- does not equal null
  ...
  this.remove = function(element) {
    ...
    var perviousNode;
    if(currentNode.element === element) { // the head node is the element that we are trying to remove
  ...
  this.addAt = function(index, element){
    ...
    if (index === 0) { // add to the head node
